...
## K-Ratio (누적 자산곡선의 추세 일관성 자체를 보는 지표 - '기울기 / 표준편차')
# %%
...
z_value = value_1.merge(value_2, on=['종목코드', 'SEC_NM_KOR']).sum(axis=1, 
...
## 모멘텀 지표의 z-score 산출
# %%
z_momentum = data_bind_group[['12M', 'K-ratio']].apply(lambda x: col_clean(x, 0.01, False)).sum(
    axis=1, skipna=False).to_frame('z_momentum')
data_bind = data_bind.merge(z_momentum, how='left', on=['종목코드', 'SEC_NM_KOR'])

data_bind.round(4).head()

## 팩터 별 분포 체크 - 기준을 많이 쓸수록 넓게 분포됨 -> 다시 z-score 산출해서 분포 넓이 비슷하게 맞춰줌
# %%
...
data_bind_final_sum.coli=umns = ['qvm']
